...
func (dao *RoomDAO) GetById(id int) (*entities.Room, error) {
	...
	dao.fillRoom(room, slots, interactives)
	return room, nil
}

func (*RoomDAO) fillRoom(room *entities.Room, slots []*entities.Slot, interactives []entities.InteractiveObject) {
	for _, slot := range slots {
		room.Slots()[slot.Id()] = slot
	}

	for _, interactive := range interactives {
		room.Interactives()[interactive.Id()] = interactive
	}
}
...
func (dao *RoomDAO) getRelatedInteractives(roomId int) ([]entities.InteractiveObject, error) {
	var ids, idsErr = dao.getRelatedEntityIds(getRoomRelatedInteractiveIdsQuery, roomId)
	if idsErr != nil {
		return nil, idsErr
	}

	var interactives = make([]entities.InteractiveObject, 0)
	for _, id := range ids {
...
func (dao *RoomDAO) getRelatedSlots(roomId int) ([]*entities.Slot, error) {
	var ids, idsErr = dao.getRelatedEntityIds(getRoomRelatedSlotIdsQuery, roomId)
	if idsErr != nil {
		return nil, idsErr
	}

	var slots = make([]*entities.Slot, 0)
	for _, id := range ids {
...
func (dao *RoomDAO) getRelatedEntityIds(query string, roomId int) ([]int, error) {
	var rows, err = dao.db.Query(query, roomId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
